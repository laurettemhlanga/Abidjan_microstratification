...
#################################################################################
#####################RAINFALL DATA IN CLIMESERV FOLDER##################################
#################################################################################
#retrieve raster files for extraction
tag <- c(3:9, 0:3)
...
saveRDS(rainfall_data, file = "rainfall_data.rds")
...
saveRDS(raindata13b, file = "rainfall_data2.rds")
extracted_rainfall <- readRDS(file = "~/Abidjanf/rainfall_data2.rds")

##units in mm according to CHIRPS site

long_rainfall_data <- df_abidjan1 %>% 
  st_drop_geometry() %>% 
  dplyr::select(NOM) %>% 
  cbind(extracted_rainfall) %>%
  reshape::melt(id = c("NOM", "ID")) %>% 
  mutate(year = substr(variable, 2, 5), 
         month = as.numeric(substr(variable, 6, 7)),
         day = as.numeric(substr(variable, 8,9))) %>% 
  group_by(NOM, month, year) %>% 
  summarize(mean_daily_rainfall = mean(value, na.rm = T)) %>% 
  ungroup() %>% 
  mutate(monthly_rainfall = 
           ifelse(month == 01|month == 03|month == 05|month == 07|month == 08|month== 10|month ==12, mean_daily_rainfall*31,
                  ifelse(month== 04 |month== 06|month== 09|month== 11, mean_daily_rainfall*30,
                         ifelse(month == 02, mean_daily_rainfall *28, NA)))) %>% 
  group_by(NOM, year) %>% 
  mutate(yearly_rainfall = as.numeric(sum(monthly_rainfall, na.rm=T)))
...
#monthly rainfall  
ggplot()+
 geom_line(data =long_rainfall_data, aes(x=month, y =monthly_rainfall, color=NOM))+
  facet_wrap(~year)+
  labs(title = "Monthly Rainfall in Abidjan 2013-2023",
       x= "Month",
       y= "Rainfall (mm)")+
...
###rainfall trend by year
ggplot()+
  geom_line(data =long_rainfall_data, aes(x=year, y =yearly_rainfall, color=NOM))+
  labs(title = "Yearly Rainfall Trend 2013-2023",
       x= "Year",
       y= "Rainfall (mm)")+
  theme_manuscript()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
ggplot()+
  geom_point(data = flood_data, aes(x= date, y= Rainfall_mm, color= HealthDistrict))+
  labs(title = "Days with floods",
       x= "Date",
       y= "Rainfall (mm)")+
  theme_manuscript()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

#daily rainfall with flood points

long_rainfall_data2 <- df_abidjan1 %>% 
  st_drop_geometry() %>% 
  dplyr::select(NOM) %>% 
  cbind(extracted_rainfall) %>%
  reshape::melt(id = c("NOM", "ID")) %>% 
  mutate(year = substr(variable, 2, 5), 
         month = as.numeric(substr(variable, 6, 7)),
         day = as.numeric(substr(variable, 8,9)),
         date = ymd(substr(variable, 2, 9))) 

ggplot()+
  geom_line(data = long_rainfall_data2, aes(x= date, y= value, color = NOM))+
  geom_point(data = flood_data, aes(x= date, y= Rainfall_mm, color= HealthDistrict))+
  labs(title = "Daily rainfall and flood points",
       x= "Date",
       y= "Rainfall (mm)",
       caption = "Highlighted points indicate flooding")+
  theme_manuscript()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
ggplot()+
  geom_line(data = overall_rainfall, aes(x= month, y =mean_monthly_rainfall, color = NOM))+
  labs(title = "Average monthly rainfall in Abidjan, 2013-2023",
       x= "Month",
       y = "Rainfall (mm)")+
  scale_x_continuous(breaks = 1:12, labels = month_labels)+
  theme_manuscript()+
...
#rainfall and malaria incidence

incidence_data <- routine_dat %>%  
  filter(grepl("ABIDJAN", health_region)) %>% 
  dplyr::select(date, month, year, health_district,confirmed_cases, allcause_outpatients, population_adjusted, incidence_adjusted0, incidence_adjusted1, incidence_adjusted2, incidence_adjusted3) 

glimpse(incidence_data)

incidence2 <- incidence_data %>% 
  #mutate(month = as.numeric(format(date, "%m"))) %>%
  group_by(month) %>%
  summarize(avg_monthly_incidence = mean(incidence_adjusted0, na.rm= T)) 

incidence3 <- incidence_data %>% 
  mutate(calc_incidence = as.numeric(confirmed_cases/ allcause_outpatients)) %>% 
  group_by(month, health_district) %>% 
  mutate(avg_incidence = median(calc_incidence, na.rm =T, trim= 0.1))

ggplot()+
  geom_point(data = incidence3, aes(x=calc_incidence, y=month, colour =health_district))+
  scale_y_continuous(breaks = 1:12, labels = month_labels)
  

#rainfall and water frequency
...
# 
# 
# selected_rainfall <-summerised_rainfall [1:15,]
# overall_rainfallplottingdata <- inner_join(df_abidjan1, selected_rainfall)
# 
# ggplot()+
#   geom_sf(data = overall_rainfallplottingdata, aes(geometry =geometry, fill =overall_rainfall))+
#   geom_text_repel(data =overall_rainfallplottingdata, 
#                 aes(label=str_to_sentence(NOM), geometry=geometry), color ='black',
#               stat = "sf_coordinates", min.segment.length = 0, size = 3.5, force = 1)+
#   scale_fill_continuous(name = "Rainfall (inches)", low = "lightyellow", high = "darkblue")+
